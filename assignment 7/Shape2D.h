...
public:
    ...
    float getWidth() { if (lowerBound.x > -INFINITY) return (upperBound.x - lowerBound.x);else{return -INFINITY;}}
    float getHeight() { if (lowerBound.x > -INFINITY) return (upperBound.y - lowerBound.y); else{return -INFINITY;}}
	...
	bool isContained(Point2D testPnt, bool isInclusive);
	//bool isContained(float testX, float testY, bool isInclusive);
