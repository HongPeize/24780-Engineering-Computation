	...
	const std::string& prompt2, const std::string& prompt3)
{
	...
	while (key != FSKEY_ESC && key != FSKEY_ENTER) {
		...
		if (prompt1.length() > 0) {
			...
			comic.drawText(prompt1, xloc, yloc, fscale);
			yloc += vspace;
			if (prompt2.length() > 0) {
				//glRasterPos2d(xloc, yloc - 10);
				//YsGlDrawFontBitmap16x20(prompt1.c_str());

				comic.drawText(prompt2, xloc, yloc, fscale);
				yloc += vspace;
				if (prompt3.length() > 0) {
					//glRasterPos2d(xloc, yloc - 10);
					//YsGlDrawFontBitmap16x20(prompt3.c_str());

					comic.drawText(prompt3, xloc, yloc, fscale);
					yloc += vspace;
				}
			}
		}
		...
		comic.setColorRGB(1., 0., 1.);
		comic.drawText(inputString, 150, yloc, fscale);
